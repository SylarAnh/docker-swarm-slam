...
bool cslam_common_interfaces__msg__inter_robot_matches__convert_from_py(PyObject * _pymsg, void * _ros_message)
{
  // check that the passed message is of the expected Python class
  {
    ...
    assert(strncmp("cslam_common_interfaces.msg._inter_robot_matches.InterRobotMatches", full_classname_dest, 66) == 0);
  }
  cslam_common_interfaces__msg__InterRobotMatches * ros_message = _ros_message;
  {  // robot_id
  ...
  {  // matches
    ...
    PyObject * seq_field = PySequence_Fast(field, "expected a sequence in 'matches'");
    if (!seq_field) {
      Py_DECREF(field);
      return false;
    }
    ...
    if (!cslam_common_interfaces__msg__InterRobotMatch__Sequence__init(&(ros_message->matches), size)) {
      PyErr_SetString(PyExc_RuntimeError, "unable to create cslam_common_interfaces__msg__InterRobotMatch__Sequence ros_message");
      Py_DECREF(seq_field);
      Py_DECREF(field);
      return false;
    }
    cslam_common_interfaces__msg__InterRobotMatch * dest = ros_message->matches.data;
    for (Py_ssize_t i = 0; i < size; ++i) {
      if (!cslam_common_interfaces__msg__inter_robot_match__convert_from_py(PySequence_Fast_GET_ITEM(seq_field, i), &dest[i])) {
        Py_DECREF(seq_field);
        Py_DECREF(field);
        return false;
      }
...
{
  /* NOTE(esteve): Call constructor of InterRobotMatches */
  PyObject * _pymessage = NULL;
  {
    PyObject * pymessage_module = PyImport_ImportModule("cslam_common_interfaces.msg._inter_robot_matches");
    assert(pymessage_module);
    PyObject * pymessage_class = PyObject_GetAttrString(pymessage_module, "InterRobotMatches");
  ...
  {  // matches
    ...
    cslam_common_interfaces__msg__InterRobotMatch * item;
    for (size_t i = 0; i < size; ++i) {
      item = &(ros_message->matches.data[i]);
      PyObject * pyitem = cslam_common_interfaces__msg__inter_robot_match__convert_to_py(item);
      if (!pyitem) {
        Py_DECREF(field);
        return NULL;
      }
